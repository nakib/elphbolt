...
To date (January 04, 2024), ~elphbolt~ / ~superconda~ has been used in the following works:
...
**** Using ~fpm~ (v>=0.7)
     For a cpu-only build and test with Fortran Package Manager (https://fpm.fortran-lang.org/), say
      ...
      #+BEGIN_SRC bash
      source fpm_config_caf.sh; yes | fpm clean; fpm build; fpm install; fpm test
      #+END_SRC
...
***** Experimental
     ...
     #+BEGIN_SRC bash
      source fpm_config_caf_openacc.sh; yes | fpm clean; fpm build; fpm install; fpm test
     #+END_SRC
