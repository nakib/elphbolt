program V3offload
  ...
  call t_event%start_timer('V- on gpu')
  call calculate_3ph_interaction_gpu(ph, crys, num, V2)
  call t_event%end_timer('V- on gpu')
  print*, 'value = ', twonorm(pack(V2, .true.))
contains
  ...
  subroutine calculate_3ph_interaction(ph, crys, num, V2, Vm2_calculator)
    ...
    integer(i64) :: istate1, nstates_irred, &
         ...
         neg_iq2, neg_q2_indvec(3), idim, jdim, s2s3
  ...
  subroutine calculate_3ph_interaction_gpu(ph, crys, num, V2)
    ...
    do istate1 = 1, nstates_irred
       ...
       ev1 = reshape(ph%evecs(iq1, s1, :), shape = [3, 2])
       ...
       do iq2 = 1, ph%nwv
          ...
          do s2 = 1, ph%numbands
             ev2 = reshape(ph%evecs(iq2, s2, :), shape = [3, 2])
             !$acc update device(ev2)
             
             do s3 = 1, ph%numbands
                ev3 = reshape(ph%evecs(iq3_minus, s3, :), shape = [3, 2])
  ...
  real(r64) function Vm2_3ph_refactor(ev1_s1, ev2_s2, ev3_s3, &
    ...
    !Local variables
    integer(i64) :: it, a, b, c, aind, bind, cind
    integer(i64) :: i, j, k, ijk, nijk, ndim
    complex(r64) :: aux1, aux2, aux3, V0
    ...
    nijk = ntrip
    ndim = 3
    ev1 = reshape(ev1_s1, shape = [3, 2])
    ev2 = reshape(ev2_s2, shape = [3, 2])
    ev3 = reshape(ev3_s3, shape = [3, 2])
