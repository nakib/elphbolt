...
program elphbolt_with_coherence
  ...
  case(1) !BTE workflow     
     ...
     call calculate_dos(ph, crys, num%tetrahedra, sep%ph_rta_rates_iso_ibz, sep%ph_rta_rates_subs_ibz, &
     ...
     if(num%onlyphbte .and. num%phe .or. num%drag) then
        ...
        !Calculate ph-e coherence transition probabilities
        call t_event%start_timer('IBZ ph-e Us')

        call calculate_eph_interaction_ibzq(wann, crys, el, ph, num, 'U')

        call t_event%end_timer('IBZ ph-e Us')
     end if
     ...
     if(num%onlyebte .or. num%drag) then
        ...
        !These are not needed in the simplified theory:
!!$        call t_event%start_timer('IBZ e-ph Omegas^+/-')
!!$
!!$        !Calculate e-ph transition probabilities
!!$        call calculate_eph_interaction_ibzk(wann, crys, el, ph, num, 'O')
!!$
!!$        call t_event%end_timer('IBZ e-ph Omegas^+/-')
     end if
     ...
     if(num%solve_bulk) then
        if(num%onlyphbte .and. .not. num%phe) then
           call sep%solve_sepe(num, crys, sym, ph)
        else
           call sep%solve_sepe(num, crys, sym, ph, el)
        end if
     end if
...
end program elphbolt_with_coherence
