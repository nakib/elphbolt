...
contains
  ...
  subroutine map_triplet_full_to_reduced(nbands, mesh_size, lambda_1, lambda_2, M)
    !! This subroutine maps the canonical triplet is converted back to (ilambda2, ilambda1) and stored in M(:, ilambda2, ilambda1).
    !!
    !! M is a 3D array: M(3, size(lambda_2), size(lambda_1))
    ...
    allocate(M(3, size(lambda_2), size(lambda_1)))
    ...
    do ilambda1 = 1, size(lambda_1)
       ...
       do ilambda2 = 1, size(lambda_2)
          ...
          ! Initialize the canonical triplet with a high value, so any triplet you compare against will be smaller
