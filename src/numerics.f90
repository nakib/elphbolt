...
contains
  ...
  subroutine read_input_and_setup(self, crys)
    ...
    logical :: read_gq2, read_gk2, read_V, read_W, read_ee_pol1, tetrahedra, phe, phiso, phsubs, &
    ...
    namelist /numerics/ qmesh, mesh_ref, fsthick, datadumpdir, read_gq2, read_gk2, &
         read_V, read_W, read_ee_pol1, tetrahedra, phe, phiso, phsubs, onlyphbte, onlyebte, maxiter, &
...
!!$       end if
    ...
    if(self%runlevel /= 3) then !Non-superconductivity mode
       ...
       self%read_V = read_V
       self%read_W = read_W
       self%read_ee_pol1 = read_ee_pol1
       self%W_OTF = W_OTF
       self%phe = phe
       self%phiso = phiso
       ...
       if(self%runlevel /= 3) then
          ...
          if(self%elel .or. self%elchimp) then
             ...
             if(self%Coulomb_screening_type == 'RPA') then
                ...
                write(*, "(A, L)") "Reuse e-e polarisability: ", self%read_ee_pol1
             end if
          end if
