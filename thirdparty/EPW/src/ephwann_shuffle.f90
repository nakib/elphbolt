  ...
  !!  
  !!  Nakib Haider Protik, April 2023 - Small modification to print out Wigner-Seitz
  !!  vectors and degeneracies for later use in elphbolt. Search for NHP
  !!  to jump to the modification block.  
  !-----------------------------------------------------------------------
  !
  ...
  !
  !NHP April 19, 2023
  !Print out irevec_k,irvec_q,irvec_g to file
  if(mpime==ionode_id) then
     open(unit=999,file='rcells_k',status='replace')
     do ir=1,nrr_k
        write(999,"(3(I10,x))") irvec_k(:,ir)
     end do
     close(999)

     open(unit=999,file='rcells_q',status='replace')
     do ir=1,nrr_q
        write(999,"(3(I10,x))") irvec_q(:,ir)
     end do
     close(999)
     
     open(unit=999,file='rcells_g',status='replace')
     do ir=1,nrr_g
        write(999,"(3(I10,x))") irvec_g(:,ir)
     end do
     close(999)

     open(unit=999,file='wsdeg_q',status='replace')
     do ir=1,nrr_q
        write(999,"(1(I10,x))") ndegen_q(ir,1,1)
     end do
     close(999)

     open(unit=999,file='wsdeg_k',status='replace')
     do ir=1,nrr_k
        write(999,"(1(I10,x))") ndegen_k(ir,1,1)
     end do
     close(999)

     open(unit=999,file='wsdeg_g',status='replace')
     do ir=1,nrr_g
        write(999,"(1(I10,x))") ndegen_g(ir,1,1,1)
     end do
     close(999)
