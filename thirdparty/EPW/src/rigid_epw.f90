  ...
  !-----------------------------------------------------------------------
  !!
  !! Nakib Haider Protik, April 2023 - Modification in subroutine rgd_blk_epw
  !! and rgd_blk_epw_fine to make the argument of the exponential of the
  !! Ewald sum dimensionless. Search for NHP to jump to the modification block.
  !!
  !----------------------------------------------------------------------
  MODULE rigid_epw
  !----------------------------------------------------------------------
      ...
      DO m2 = -nqc2, nqc2
        DO m3 = -nqc3, nqc3
          ...
          IF (qeq > 0.0d0 .AND. qeq / (alph * 4.0d0) < gmax) THEN
            !
            !
            !NHP: Uncommenting the following line will make the argument of exp() dimensionful. 
            !qeq = qeq * twopi / alat
